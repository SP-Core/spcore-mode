...
public class ParameterVector2 extends AbstractNodeType {
    ...
    protected HashMap<String, Object> internalProcess(Node node, ProcessService inputs) {
        ...
        var v = new ParameterValue<Vector2f>();

        if(!node.containsInputValue("id")){
            outputs.put("value", new Vector2f());
        ...
        if(inputs.contains("input")){
            ...
            v.value = (Vector2f) inputMap.get(id);
        }
        else{
            if(inputs.contains("value")){
                v.value = (Vector2f) inputs.process("value");
            }
